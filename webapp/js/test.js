...
function initScene() {
    ...
    camera.position.set(0, 2, 0).multiplyScalar(6);//위에서 아래보기
    camera.lookAt(0, 0, 0);
...
function createWalls() {
    ...
    // 벽과 주사위 간의 마찰 설정
    const diceMaterial = new CANNON.Material('diceMaterial'); // 주사위 재질
    const wallDiceContactMaterial = new CANNON.ContactMaterial(wallMaterial, diceMaterial, {
        friction: 0.0001,      // 마찰 계수
        restitution: 10    // 튕김 계수
    });

    physicsWorld.addContactMaterial(wallDiceContactMaterial);
}
